...
class Route53Service(BaseAWSService):
    ...
    def list_hosted_zone_by_tags(self):
        ...
        if matching_zones:
            ...
            return "There is no Hosted zones"

    # Update record
    def update_resource_record(self, zone_id, record_name, value, ttl):
    ...
    # Delete DNS resource
    def delete_resource_record(self,zone_id, record_name, value, ttl):
        if zone_id in self.list_hosted_zone_by_tags():
            try:
                response = self.client.change_resource_record_sets(
                    ChangeBatch={
