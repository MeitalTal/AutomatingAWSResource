from modules.BaseAWSService import BaseAWSService
    ...
    def create_instance(self, instance_name, instance_type, ami):
        ...
        if self.count_of_running_instance() < 2:
            try:
                ...
                print(f"The instant has been launched successfully! \n Instance ID: {launched_instance.id} \n  Public IP: {launched_instance.public_ip_address}")
            except Exception as e:
                print(f"Error creating instance: {e}")
        else:
            print("Sorry! you can have just 2 running instances!")



    # Get List of Instances that created by CLI
    def print_list_of_instances(self):
        if not list(self.instances()):
            print("There is no instances")
            return
        ...
        for instance in self.instances():
            if instance.state['Name'] == actions[action] and instance.id == instance_id:
                try:
                    ...
                    method()
                    print(f"Instance {instance_id} is {action}ing...")
                    return "Successful!"
                except Exception as e:
                    print(f"Failed to {action} instance {instance_id}: {e}")
                    return "Failed!"
        return f"Instance {instance_id} not found or not in '{actions[action]}' state."